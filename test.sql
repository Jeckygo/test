...
#3、查询平均成绩大于等于60分同学的学生编号和学生姓名和平均成绩
SELECT st.s_id,st.s_name,ROUND(AVG(sc.s_score),1)
FROM student st
...
SELECT st.s_id,st.s_name,COUNT(sc.c_id),SUM(case WHEN sc.s_score is NULL then 0 ELSE sc.s_score END)
FROM student st
